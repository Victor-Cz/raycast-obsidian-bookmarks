...
- Save the site's human name, read from the page's `og:site_name`, as the `publisher` frontmatter field — so search results show "GitHub" rather than "github.com". Pages that name no site fall back to the domain as before, and editing a bookmark keeps the value its note already had.
